...
class ImageProcessorThread(QThread):
    ...
    def run(self):
        try:
            ...
            self.finished.emit(documents, processing_time, chosen_model)
...
class VisionToolSettingsTab(QWidget):
    ...
    def onProcessingFinished(self, documents, processing_time, chosen_model):
        self.thread = None
        print(f"Processed {len(documents)} documents")
        contents = self.extract_page_content(documents)
        self.save_page_contents(contents, processing_time, chosen_model)
    ...
    def save_page_contents(self, contents, processing_time=0.0, chosen_model="Unknown"):
